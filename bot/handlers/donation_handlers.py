# ===============================
# bot/handlers/donation_handlers.py - תרומות TON
# ===============================
...
def build_donation_keyboard() -> InlineKeyboardMarkup:
    ...
    בניית מקלדת תרומות
    """
    # קישור ישיר לתרומה ב-Tonkeeper או Wallet
    ton_link = f"ton://transfer/{TON_WALLET_ADDRESS}"
    
    buttons = [
        [InlineKeyboardButton(
            f"💎 לתרום {MIN_DONATION_AMOUNT} TON",
            url=f"{ton_link}?amount={int(MIN_DONATION_AMOUNT * 1e9)}"
        )],
        [InlineKeyboardButton(
            "💎 לתרום 5 TON",
            url=f"{ton_link}?amount={int(5 * 1e9)}"
        )],
        [InlineKeyboardButton(
            "💎 לתרום 10 TON",
            url=f"{ton_link}?amount={int(10 * 1e9)}"
        )],
        [InlineKeyboardButton(
            "💎 לתרום סכום מותאם אישית",
            url=ton_link
        )],
        [InlineKeyboardButton(
            "📋 העתקת כתובת ארנק",
...
async def check_donation_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # כרגע מידע סטטי - אפשר לשלב עם TON API בעתיד
    text = (
        "📊 סטטוס תרומות:\n\n"
...
async def notify_donation_received(context: ContextTypes.DEFAULT_TYPE, amount: float, sender: str = "Unknown"):
    ...
    (זה ידרוש אינטגרציה עם TON API בעתיד)
    """
    if not LOG_GROUP_ID:
        return
